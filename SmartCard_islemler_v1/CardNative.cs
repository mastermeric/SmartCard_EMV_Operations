...
namespace SmartCard_islemler_v1
{
    ...
    public class CardNative : CardBase
    {
        ...
        [DllImport("winscard.dll", SetLastError = true)]
        internal static extern uint SCardStatus(UInt32 hCard,
            StringBuilder szReaderName,
            ref UInt32 pcchReaderLen,
            ref UInt32 pdwState,
            ref UInt32 pdwProtocol,
            [Out] byte[] pbAtr,
            ref UInt32 pcbAtrLen);

        [DllImport("winscard.dll", SetLastError = true)]
        internal static extern uint SCardReconnect(UInt32 hCard,
            UInt32 dwShareMode,
            UInt32 dwPreferredProtocols,
        ...
        public override APDUResponse Transmit(APDUCommand ApduCmd)
        {
            ...
            string LogDosyasi = @"c:\bberror\PERSO_TEST_Log_" + DateTime.Now.ToShortDateString().Replace("/", "").Replace(":", "").Replace(" ", "") + ".txt";
            {
                if (!Directory.Exists(@"c:\bberror")) Directory.CreateDirectory(@"c:\bberror");
                if (!File.Exists(LogDosyasi))
                {
                    FileStream fs = null;
                    fs = File.Create(LogDosyasi);
                    fs.Close();
                }
            }
            ...
            else if (ApduCmd.Data != null && ApduCmd.Le == 255)
            {
                ApduBuffer = new byte[APDUCommand.APDU_MIN_LENGTH + 1 + ApduCmd.Data.Length + 1];
                ...
                ApduBuffer[APDUCommand.APDU_MIN_LENGTH + 1 + ApduCmd.Data.Length] = (byte)0;
            ...
            {
                sw.WriteLine("Realtime-SCardTransmit() ----->   APDU Transmit LOGLAMA:");
                for (int i = 0; i < ApduBuffer.Length; i++)
                {
                    sw.WriteLine("ApduBuffer[" + i.ToString() + "]  = " + ApduBuffer[i].ToString("X02"));
                }
            }
        ...
        public override bool CardStatus(string Reader, ref byte[] ATR, ref uint pcbAtrLen)
        {
            uint ReaderLen = 300, pdwState = 0;
            StringBuilder sb = new StringBuilder(Reader);

            SCard_IO_Request ioRequest = new SCard_IO_Request();
            ioRequest.m_dwProtocol = m_nProtocol;

            uint ret = SCardStatus(m_hCard, sb, ref ReaderLen, ref pdwState,
                                    ref ioRequest.m_dwProtocol, ATR, ref pcbAtrLen);
        ...
        public override bool CardReConnect(int DisconnectType)
        {
            uint dwATRLength = 40;
            ...
            uint ret = SCardReconnect(m_hCard, (uint)SHARE.Shared, ioRequest.m_dwProtocol,
                                        (uint)DisconnectType, out dwATRLength);
